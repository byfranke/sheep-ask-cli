...
# Run the installer and setup wizard
...
sudo cp sheep-ask-cli.py /usr/local/bin/sheep-ask
sudo chmod +x /usr/local/bin/sheep-ask
```
...
```bash
./sheep-ask-cli.py --token "your_api_token_here" "What is ransomware?"
```
...
./sheep-ask-cli.py "What is ransomware?"
```
...
./sheep-ask-cli.py What are the TTPs of APT29

# Explain a framework
./sheep-ask-cli.py "Explain the MITRE ATT&CK framework"
```
...
./sheep-ask-cli.py -o "What is a zero-day vulnerability?"
```
...
./sheep-ask-cli.py -p report.md "Summarize the key findings"
./sheep-ask-cli.py --prompt incident.md "What are the IOCs mentioned?"
```
...
./sheep-ask-cli.py "Explain phishing"

# Plain text for piping / automation
./sheep-ask-cli.py "Explain phishing" --format plain

# Raw JSON
./sheep-ask-cli.py "Explain phishing" --format json

# Rendered markdown (no panel)
./sheep-ask-cli.py "Explain phishing" --format markdown
```
...
```bash
# Show help
./sheep-ask-cli.py --help

# Show version
./sheep-ask-cli.py --version

# Re-run the setup wizard
./sheep-ask-cli.py --setup

# Check for updates from GitHub
./sheep-ask-cli.py --update
```
